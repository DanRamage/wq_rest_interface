...
$.widget( "mobile.tabs", $.mobile.widget, {
  ...
  _create: function(){
    ...
    function navButtonClick(event) {
      ...
      $('#detail_map_canvas').gmap('refreshCenter'); //ugly hack - inserted to refresh map, but this should be done in the mbsa.js file
      $('#beachDetailsPage').trigger('pageshow'); //ugly again - needed to handle redraw of graph if screen rotated while map tab was selected
      $('body').removeClass('ui-loading'); // also ugly - remove loading indicator that is added on every 'a' click event
    }
